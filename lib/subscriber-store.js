...
function dbUrl() {
  const direct =
    ...
    // Supabase integration often provides this pooled URL for serverless apps.
    process.env.DATABASE_POSTGRES_PRISMA_URL ||
    process.env.DATABASE_POSTGRES_URL ||
    '';
  if (direct) return direct;

  // Fallback for environments that expose separate Postgres connection parts.
  const host = process.env.DATABASE_POSTGRES_HOST || '';
  const user = process.env.DATABASE_POSTGRES_USER || '';
  const pass = process.env.DATABASE_POSTGRES_PASSWORD || '';
  const db = process.env.DATABASE_POSTGRES_DATABASE || '';
  if (host && user && pass && db) {
    return `postgresql://${encodeURIComponent(user)}:${encodeURIComponent(pass)}@${host}:5432/${encodeURIComponent(
      db
    )}?sslmode=no-verify`;
  }
  return '';
...
function sanitizeDbUrl(raw) {
  ...
  try {
    ...
    // Strip certificate path params that can cause CA chain issues in serverless.
...
function getPool() {
  if (!pool) {
    const connectionString = sanitizeDbUrl(dbUrl());
